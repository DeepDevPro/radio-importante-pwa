...
app.get('/api/catalog', async (req, res) => {
  // Catálogo já inicializado no bootstrap (initializeCatalog). Mantemos fallback simples apenas logando estado vazio.
  if (catalog.tracks.length === 0) {
    console.log('📖 [catalog] Aviso: catálogo ainda vazio após inicialização (pode ser primeiro deploy)');
  }
  res.json(catalog);
});
...
app.post('/api/upload', flexibleUpload, async (req, res) => {
  ...
  try {
    ...
    try {
      // TODO(Etapa5): chamar generateContinuousFile após extração para service
